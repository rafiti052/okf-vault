...
Initialize a new OKV vault at `./knowledge/` then run a quality gate on vault health.
...
- **Helper CLI** — agents invoke `okv init --json` then `okv validate --json` with argument arrays when helper output is needed
