...
- The helper creates ordered turn-level source spans under `references/sources/<source-slug>/span-XXX.md` from usable speaker and timestamp anchors.
- A span records `anchor_kind: timestamp-speaker` when a matching speaker marker and timestamp form one turn; timestamp-only and speaker-only turns remain explicit fallbacks.
- Available `speaker` and `timestamp` values are preserved in span metadata and rendered evidence. The helper does not invent missing speakers, timestamps, or a new turn order.
- Span sequence follows envelope anchor order, with immediate `prev` and `next` links between adjacent turns. Post-selection hydration returns the exact turn plus at most one previous and one next turn while preserving attribution; panel spans are never first-hop retrieval candidates.
