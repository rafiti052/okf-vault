...
- YouTube URL-first MVP sources (`kind: youtube`) follow the same timestamp fidelity rules; default profile is **video** unless the curator confirmed **panel** during acquisition.
...
- The helper creates one timestamp source span per usable `timestamp` anchor under `references/sources/<source-slug>/span-XXX.md`.
- Span sequence follows envelope transcript order. The helper preserves each supplied timestamp and never sorts segments or invents finer timing.
- Every span carries `profile: video`, `anchor_kind: timestamp`, its timestamp, and the originating timestamp anchor ID.
- Immediate `prev` and `next` links model adjacent transcript segments. Post-selection hydration returns the exact timestamp segment plus at most one previous and one next segment; raw transcript spans are never first-hop retrieval candidates.
...
| YouTube transcript with clear single-narrator shape | **Video** (MVP default)                       |
| YouTube transcript ambiguous multi-speaker shape    | **Panel** only after explicit curator confirmation during acquisition |
